...
from controller.conformal_cvar_cbf_controller import ConformalDCLFCVARDCBF
from controller.sfm import SFM
from controller.nominal import Nominal
from controller.orca import ORCA
from controller.orca_robot import ORCARobot
from controller.crowdnav_controller import CrowdNavController

from util.util import *
# from util.animation import *
from util.animation_modified import *
...
@click.option(
    "--S", "S", type=int, default=20, show_default=True,
    help="Number of samples in the uncertainty distribution. Higher S increases risk accuracy."
)
@click.option(
    "--beta", "beta", type=float, default=0.3, show_default=True,
...
@click.option(
    "--time-total", "time_total", type=float, default=30.0, show_default=True,
    help="Total simulation time in seconds."
)
@click.option(
    "--ctrl-type", "ctrl_type", type=str, default="cbf", show_default=True,
    help="Controller type (cbf, cvarbf, adap_cvarbf, conformal_cvarbf, orca, orca_robot, crowdnav)."
)
def main(htype, S, beta, save_ani, time_total, ctrl_type):
    ...
    config_folder = SCRIPT_DIR + '/config/' + '20obs/'  # Complex scenario - may need parameter tuning
    # config_folder = SCRIPT_DIR + '/config/' + 'one_obs/'  # Reliable baseline test
    ...
    while not done:
        ...
        for robot, controller in zip(env.robots, controllers):
            ...
            loc_obstacles = robot.update_local_objects(env.obstacles, env.robots, R_sensing=15)
...
def create_controllers(env, ctrl_type, params):
    ...
    controller_mapping = {
        ...
        "conformal_cvarbf": ConformalDCLFCVARDCBF,  # Conformal CVaR-CBF with experience-based adaptation
        "orca": ORCA,  # ORCA collision avoidance (dual-interface)
        "orca_robot": ORCARobot,  # ORCA collision avoidance (robot-only)
        "crowdnav": CrowdNavController,  # CrowdNav trained neural policy
    }
...
def create_obs_controller(env):
    ...
    controller_mapping = {
        "sfm": SFM,
        "nominal": Nominal,
        "orca": ORCA,  # ORCA collision avoidance for obstacles
    }
    obs_controllers = []
