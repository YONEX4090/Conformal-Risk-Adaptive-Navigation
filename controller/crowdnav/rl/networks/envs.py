...
# Try to import from baselines, create fallback if not available
try:
    from baselines import bench
    from baselines.common.atari_wrappers import make_atari, wrap_deepmind
    from baselines.common.vec_env import VecEnvWrapper
except ImportError:
    # Create a simple Monitor class as fallback
    class Monitor:
...
try:
    ...
    # Fallback VecNormalize implementation
    class VecNormalize_:
        def __init__(self, venv, ret=True, ob=True, gamma=0.99, epsilon=1e-8, clipob=10.0):
            self.venv = venv
            self.ret = ret
            self.ob = ob
            self.gamma = gamma
...
try:
    ...
    # Create a fallback if the module is not available
    class VecPretextNormalize:
...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, config=None, envNum=1, ax=None, test_case=-1):
    def _thunk():
        if env_id.startswith("dm"):
            ...
            # Disable gym's env checker to avoid issues with uninitialized action_space
        ...
        # Configure the environment first before accessing action_space/observation_space
