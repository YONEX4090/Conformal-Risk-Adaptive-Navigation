...
def evaluate(actor_critic, eval_envs, num_processes, device, test_size, logging, config, args, visualize=False):
    ...
    def get_base_env(envs):
        """Recursively unwrap environment wrappers to get the base environment"""
        current = envs
        # Try different access patterns for different wrapper types
        paths_to_try = [
            ['venv', 'envs', 0, 'env'],           # Standard path
            ['venv', 'unwrapped', 'envs', 0, 'env'], # With unwrapped
            ['venv', 'venv', 'envs', 0, 'env'],   # Double wrapped
            ['venv', 'envs', 0],                  # Direct access to env
            ['envs', 0, 'env'],                   # Alternative path
            ['envs', 0]                           # Direct env access
        ]
        ...
        for path in paths_to_try:
            try:
                ...
                # Check if we found a valid environment
                if hasattr(obj, 'time_limit'):
                    return obj
            except (AttributeError, IndexError, KeyError):
                continue
        
        # Fallback: return the current object and hope it has time_limit
        return current
    
    baseEnv = get_base_env(eval_envs)
